...
if bPressureModel == 1
    ...
    [~,~,ke,E,~,~,~,~,~,~,~,~]    = Initialize_Thermodynamics(bSolver,P,T,u,v,w,bPressureModel,Fluid,Substance);
    rhoE = rho.*E;
else
...
if bPressureModel == 1
    [~, ~, ~, T, c_p, c_v, gamma, sos, Beta_T, Beta_v, Beta_s, Alpha_p] = Update_ThermodynamicState(bSolver,rho,u,v,w,E,P,T,Fluid,Substance,bPressureModel);
    ...
    for t = 1:n_iter_max
        ...
        for i_RK = 1:RK_order
            ...
            if bPressureModel == 1
                ...
                % Calculate total energy (induced quantity)
                e     = Calculate_e_from_TandPandRho(bSolver, T,rho,P,Fluid, Substance );
                E     = e + ke;
                rhoE  = rho.*E;
            else

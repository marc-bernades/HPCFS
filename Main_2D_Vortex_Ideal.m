...
Fluid.Ma          = 0.1;                    % Mach number
Fluid.P_0         = 1E5;                    % Reference pressure [Pa]
Fluid.rho_0       = 1;                      % Reference density
Fluid.T_0         = Fluid.P_0/(Fluid.rho_0*Fluid.R_specific);              % Reference temperature [k]
Fluid.U_0         = Fluid.Ma*sqrt(Fluid.gamma*Fluid.R_specific*Fluid.T_0); % Reference velocity [m/s]
Fluid.mu_0        = 0.0;                    % Dynamic viscosity of the fluid [Pa s]
Fluid.kappa_0     = 0.0;                    % Thermal conductivity of the fluid
Fluid.Ma_vortex   = Fluid.Ma;               % Vortex Ma number
Fluid.r_v         = 0.2;                    % Vortex radius core ref Pirozzoli L/r_v = 24
Fluid.Vortex_x0   = 0.0;                     % Vortex center
Fluid.Vortex_y0   = 0.0;                     % Vortex center

% Problem parameters
L_x           = 24*Fluid.r_v;               % Size of domain in x-direction [m]
L_y           = 12*Fluid.r_v;               % Size of domain in y-direction [m]
L_z           = 0.01*Fluid.r_v;             % Size of domain in z-direction [m]
x_0           = -L_x/2;                     % Domain origin in x-direction [m]
y_0           = -L_y/2;                     % Domain origin in y-direction [m]
z_0           = -L_z/2;                     % Domain origin in z-direction [m]
t_0           = 0.0;                        % Initial time [s]
t_c           = L_x/Fluid.U_0;              % Reference FTT
n_FlowTimes   = 1;                          % Number FFT
t_end         = t_c*n_FlowTimes;            % Final time [s]
Test          = '2D_Vortex';                % Test assessment for Initial conditions
...
output_iter = 0:t_c:t_end;        % Output data every selected time step
...
% Solution Plots
[u_exact,v_exact, rho_exact, P_exact] = Calculate_Exact_2D_Vortex(X,Y,Z,time,mu,rho,Fluid);
plot_2D_Vortex(u,v,w,X,Y,Z,L_x,L_y,L_z,P, num_grid_x, num_grid_y, num_grid_z, u_exact, v_exact, Fluid);

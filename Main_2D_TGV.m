...
Fluid.T_0         = 300;                    % Reference temperature [k]
Fluid.rho_0       = Fluid.P_0/(Fluid.R_specific*Fluid.T_0);                % Reference density
...
L_z           = 0.01*2*pi;                  % Size of domain in z-direction [m]
t_0           = 0.0;                        % Initial time [s]
t_c           = max([L_x,L_y,L_z])/Fluid.U_0; % Reference time
t_end         = 0.1*t_c;                      % Final time [s]
Test          = '2D_TGV';                   % Test assessment for Initial conditions

% Computational parameters
num_grid_x  = 20;
num_grid_y  = 20;
num_grid_z  = 1;
CFL         = 0.5;                  % CFL time-step stability coefficient
n_iter_max  = 1E10;                 % Maximum number of time iterations
output_iter = 0:t_c/2:t_end;        % Output data every selected time step
...
% Solution Plots
[u_exact,v_exact] = Calculate_Exact_2D_TGV(X,Y,Z,time,mu,rho,Fluid.U_0);
plot_2D_TGV(u,v,w,X,Y,Z,L_x,L_y,L_z,Fluid.U_0, num_grid_x, num_grid_y, num_grid_z, u_exact, v_exact, Fluid);
